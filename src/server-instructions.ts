...
export function buildInstructions(userId?: string): string {
  ...
  return [
    ...
    'Re-reading a task you already read this session: pass comments_only true. The metadata',
    'and description have not changed and re-sending them is the single largest waste here.',
    '',
    'FEWER ROUND TRIPS',
    'Several known task IDs: one list_tasks with task_ids, not get_task per ID.',
    'Several attachments: one get_attachment with attachment_ids, not one call each.',
    'Replying and handing a task back: add_task_comment with assignee_id, not a follow-up',
    'update_task_assignment.',
    'A batch of timesheet entries: one create_time_entries, not create_time_entry per row.',
    'Before booking time, check the service has time tracking enabled: get_project_services',
    'prints it, and booking against a disabled service fails after you have written the note.',
    '',
    'USER CONTEXT',
    userContext,
  ].join('\n');
